...
ros2socketcan::ros2socketcan()
  ...
  publisher_ =
      this->create_publisher<can_msgs::msg::Frame>(topicname_receive.str(), 10);
  subscription_ = this->create_subscription<can_msgs::msg::Frame>(
      topicname_transmit.str(), 100,
...
void ros2socketcan::stop() {
  RCLCPP_INFO(this->get_logger(), "End of Listener Thread. Please press strg+c "
                                  "again to stop the whole program.\n");
  ios.stop();
  signals.clear();
}
...
void ros2socketcan::CanSend(const can_msgs::msg::Frame msg) {
  ...
  out << std::string("S | ") << std::to_string(frame1.can_id)
      << std::string("| ");
...
void ros2socketcan::CanListener(
  ...
  s << std::string("R | ") << std::to_string(rec_frame.can_id)
    << std::string(" | ");
